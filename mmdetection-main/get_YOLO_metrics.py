...
def parse_opt():
    ...
    parser.add_argument('--label_coco', type=str, default='datasets/VisDrone2019-DET-test-dev/annotations/test.json',
                        help='label coco path')
    # parser.add_argument('--pred_coco', type=str, default='runs/val/exp/predictions.json', help='pred coco path')
    parser.add_argument('--pred_coco', type=str, default='cascade_test_11_13.pkl', help='pred coco path')
...
if __name__ == '__main__':
    ...
    if pred_coco_json_path.endswith('json'):
        ...
        # 建立文件名到数字ID的映射
        filename_to_id = {}
        for img_info in label['images']:
            # 假设标签中的文件名格式与预测文件一致
            filename = os.path.splitext(img_info['file_name'])[0]  # 去掉扩展名
            filename_to_id[filename] = img_info['id']

        print(f"建立的映射关系示例: {list(filename_to_id.items())[:3]}")
        ...
        for data in tqdm.tqdm(pred, desc='Process pred...'):
            # 从图像路径中提取文件名（去掉扩展名）
            filename = os.path.splitext(os.path.basename(data['img_path']))[0]

            # 将文件名映射为数字ID
            if filename in filename_to_id:
                image_id = filename_to_id[filename]  # 使用映射后的数字ID
            else:
                # 如果找不到映射，尝试其他方法或跳过
                unmatched_count += 1
                continue
        ...
        print(f"成功匹配的图像数量: {len(pred_id_dict)}")
        print(f"未能匹配的图像数量: {unmatched_count}")
