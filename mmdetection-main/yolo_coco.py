...
# python yolo2coco.py --root_dir datasets/VisDrone2019-DET-train --save_path train.json
# python yolo2coco.py --root_dir datasets/VisDrone2019-DET-val --save_path val.json
# python yolo2coco.py --root_dir datasets/VisDrone2019-DET-test-dev --save_path test.json
...
def yolo2coco(arg):
    ...
    assert os.path.exists(classes_path), f"classes.txt not found in {root_path}"
    ...
    for f in os.listdir(originImagesDir):
        if f.lower().endswith(image_extensions):
            img_name, img_ext = os.path.splitext(f)
            image_info.append((img_name, img_ext))  # 存储（主体名, 后缀）元组
    assert len(image_info) > 0, f"No images found in {originImagesDir} with extensions {image_extensions}"
    ...
    if arg.random_split or arg.split_by_file:
        ...
        if arg.random_split:
            ...
            # 仅传递文件名主体用于划分
            train_img, val_img, test_img = train_test_val_split_random([info[0] for info in image_info], 0.8, 0.1, 0.1)
        elif arg.split_by_file:
            print("spliting mode: split by files")
            train_img, val_img, test_img = train_test_val_split_by_files([info[0] for info in image_info], root_path)
    else:
    ...
    ann_id_cnt = 0
    for k, (img_name, img_ext) in enumerate(tqdm(image_info)):
        # 拼接带原始后缀的完整文件名
        img_file = f"{img_name}{img_ext}"
        txtFile = f"{img_name}.txt"

        # 读取图像宽高
        img_path = os.path.join(originImagesDir, img_file)
        ...
        if arg.random_split or arg.split_by_file:
            ...
            elif img_name in val_img:
                dataset = val_dataset
            elif img_name in test_img:
                dataset = test_dataset

        # COCO的file_name仅存主体名（无后缀）
        ...
        with open(label_path, 'r') as fr:
            ...
            for label in labelList:
                ...
                if len(label) != 5:
                    print(f"Warning: Invalid label format in {label_path}, skipping line: {label}")
                    continue
...
def yolo2coco(arg):
    ...
    for k, index in enumerate(tqdm(indexes)):
        # 支持 png jpg 格式的图片。
        txtFile = index.replace('images', 'txt').replace('.jpg', '.txt').replace('.jpeg', '.txt')
        # 读取图像的宽和高
